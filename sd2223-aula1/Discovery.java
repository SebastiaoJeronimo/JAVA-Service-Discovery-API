...
class DiscoveryImpl implements Discovery {
	...
	static final int DISCOVERY_RETRY_TIMEOUT = 30000;
	static final int DISCOVERY_ANNOUNCE_PERIOD = 30000;
	...
	private Map<String, URI[]> discoveries;
	...
	public URI[] knownUrisOf(String serviceName, int minEntries) {
		URI[] uri = discoveries.get(serviceName);
		//for(;;)
			//if (servicesOfType != null && servicesOfType.size() >= minEntries ){
				return uri;
			/* } else {
				try {
	...
	private void startListener() {
		...
		if(discoveries == null) discoveries = new HashMap<>();
		new Thread(() -> {
				...
				for (;;) {
					try {
						...
						if (parts.length == 2) {
							...
							URI[] uris = {uri};
							discoveries.put(serviceName, uris);
						}
